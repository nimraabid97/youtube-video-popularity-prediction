...
def load_predict_number_dataset(csv_path):
    ...
    comment_cols = [i for i in range(len(headers)) if headers[i] == 'comments_count']
    positive_cols = [i for i in range(len(headers)) if headers[i] == 'positive_videos']
    negative_cols = [i for i in range(len(headers)) if headers[i] == 'negative_videos']
    ...
    positive_count = np.array(pd.read_csv(csv_path, usecols = positive_cols)).ravel()
    negative_count = np.array(pd.read_csv(csv_path, usecols = negative_cols)).ravel()

    return views, likes, comment_count, positive_count, negative_count
...
def get_label(csvpath, view_bar, para_bar):
    ...
    """
    views, likes, comments_count, positive_videos, negative_videos = load_predict_number_dataset(csvpath)
    parameter = zlq.get_para(views, likes, comments_count, positive_videos, negative_videos)
